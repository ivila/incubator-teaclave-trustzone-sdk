...
fn panic_handler(panic: &core::panic::PanicInfo<'_>) -> ! {
    ...
    unsafe { optee_utee_sys::TEE_Panic(optee_utee_sys::TEE_ERROR_BAD_STATE) };
    loop {}
}
...
fn invoke_command(cmd_id: u32, params: &mut Parameters) -> Result<()> {
    match Command::try_from(cmd_id) {
        Ok(Command::Train) => {
            ...
            let mut trainer = TRAINER.lock();
            let result = trainer.as_mut().unwrap().train(bytemuck::cast_slice(images), labels);
        ...
        Ok(Command::Valid) => {
            ...
            let result = trainer.as_ref().unwrap().valid(bytemuck::cast_slice(images), labels);
        ...
        Ok(Command::Export) => {
            let trainer = TRAINER.lock();
            let result = trainer.as_ref().unwrap().export().map_err(|err| {
                trace_println!("unexpected error: {:?}", err);
                ErrorKind::BadState
            })?;
