...
use model::{images_to_tensors, Model};
use optee_utee::{
...
static MODEL: Mutex<Option<Model<NdArray>>> = Mutex::new(Option::None);
...
fn copy_to_output(param: &mut Parameter, data: &[u8]) -> Result<()> {
    ...
    if buffer.len() < data.len() {
        trace_println!("expect output buffer size {}, got size {} instead", data.len(), buffer.len());
...
fn invoke_command(_cmd_id: u32, params: &mut Parameters) -> Result<()> {
    ...
    let input = images_to_tensors(&DEVICE, images);
