...
pub fn execute(args: &Args) {
    ...
    let mut ctx = optee_teec::Context::new().unwrap();
...
fn handle(caller: &mut crate::tee::Model<'_>, request: &mut Request) -> anyhow::Result<HttpResponse> {
...
fn handle_image(caller: &mut crate::tee::Model<'_>, request: &mut Request) -> anyhow::Result<HttpResponse> {
...
fn handle_binary(caller: &mut crate::tee::Model<'_>, request: &mut Request) -> anyhow::Result<HttpResponse> {
    let mut data = Vec::with_capacity(IMAGE_SIZE);
    request.as_reader().read_to_end(&mut data)?;
...
fn handle_infer(caller: &mut crate::tee::Model<'_>, image: &[u8]) -> anyhow::Result<u8> {
