...
/// Generator of head file, use it to generate a header file and then included in user codes.
///
...
/// let uuid: &str = "26509cec-4a2b-4935-87ab-762d89fbf0b0";
/// let ta_config = TAConfig::new_standard("0.1.0", "example", "example");
/// let codes = HeaderFileGenerator::new(RustEdition::Before2024).generate(&ta_config, uuid)?;
/// # Ok(())
/// # }
/// ```
...
impl HeaderFileGenerator {
    ...
    pub fn generate(mut self, conf: &TAConfig, uuid: &str) -> Result<String, Error> {
        self.write_includes();
        self.write_configurations(conf);
        self.write_trace(conf);
        self.write_properties(conf)?;
        self.write_ta_head(conf, uuid)?;
        self.write_ta_heap();
...
impl HeaderFileGenerator {
    ...
    fn write_ta_head(&mut self, conf: &TAConfig, uuid_str: &str) -> Result<(), Error> {
        let uuid_value_codes = string_to_tee_uuid_value_codes(uuid_str)?;
...
fn property_value_data_codes(value: &PropertyValue) -> Result<proc_macro2::TokenStream, Error> {
    match value {
        ...
        PropertyValue::UUID(v) => string_to_tee_uuid_value_codes(v.as_str()),
        PropertyValue::Str(v) => Ok(string_to_binary_codes(v)),
        PropertyValue::BinaryBlock(v) => Ok(string_to_binary_codes(v)),
        PropertyValue::Identity(login, uuid) => identity_to_tee_identity_value_codes(login.clone(), &uuid),
    }
}

fn string_to_tee_uuid_value_codes(uuid_str: &str) -> Result<proc_macro2::TokenStream, Error> {
...
fn identity_to_tee_identity_value_codes(login: u32, uuid_str: &str) -> Result<proc_macro2::TokenStream, Error> {
    let tee_uuid_codes = string_to_tee_uuid_value_codes(uuid_str)?;
    Ok(quote! {
...
mod tests {
    ...
    fn test_edition_before_2024() {
        let conf = TAConfig::new_standard(
            "0.1.0",
            "test_before_2024",
            "test before 2024",
        );
        ...
        let codes = generator.generate(&conf, uuid).unwrap();
