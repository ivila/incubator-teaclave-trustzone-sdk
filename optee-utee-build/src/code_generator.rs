...
impl HeaderFileGenerator {
    fn new(edition: RustEdition) -> Self {
        Self {
            code: quote!(),
            edition,
        }
    }
    fn generate(mut self, conf: &TAConfig, uuid: &str) -> Result<String, Error> {
        self.write_includes();
...
fn property_value_data_codes(value: &PropertyValue) -> Result<proc_macro2::TokenStream, Error> {
    match value {
        ...
        PropertyValue::Identity(_login, _uuid) => todo!(),
    }
}
...
pub fn generate(edition: RustEdition, conf: &TAConfig, uuid_str: &str) -> Result<String, Error> {
    HeaderFileGenerator::new(edition).generate(conf, uuid_str)
}
