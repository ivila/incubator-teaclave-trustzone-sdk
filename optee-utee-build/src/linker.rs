...
/// The type of the linker, there are difference when using gcc/cc or ld/lld as linker,
/// For example, `--sort-section=alignment` parameter changes to `-Wl,--sort-section=alignment`
...
///
/// Use only if you just want to handle the linking stuff, and use a hand written user_ta_header.rs,
...
/// ```
///
/// We detect the type of the linker automatically, you can set it manually if you met some
/// problems with it.
/// ```no_run
...
impl Linker {
    ...
    fn write_and_set_linker_script(&self, out_dir: PathBuf, ta_dev_kit_dir: PathBuf) -> Result<(), Error> {
    ...
    fn auto_detect_linker_type() -> LinkerType {
        ...
        match env::var(ENV_RUSTC_LINKER) {
            Ok(ref linker_name) if linker_name.ends_with("ld") || linker_name.ends_with("ld.bfd") => {
